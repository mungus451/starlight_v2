...
class BlackMarketService
{
    ...
    */
    ...
    public function openVoidContainer(int $userId): ServiceResponse
        ...
        case 'unit':
        // Dynamic Unit Handling (Soldiers, Guards, Spies, Sentries)
        $u = $selected['unit'];
        ...
        if ($u === 'soldiers') $newSoldiers += $qty;
        elseif ($u === 'guards') $newGuards += $qty;
        elseif ($u === 'spies') $newSpies += $qty;
        elseif ($u === 'sentries') $newSentries += $qty;
        ...
        case 'unit_loss':
        $u = $selected['unit'];
        $currentUnits = $resources->{$u} ?? 0;
        $newUnits = max(0, $currentUnits - $qty);
        $loss = $currentUnits - $newUnits;

        // Initialize current counts for update
        $finalSoldiers = $resources->soldiers;
        $finalGuards = $resources->guards;
        $finalSpies = $resources->spies;
        $finalSentries = $resources->sentries;

        // Apply loss
        if ($u === 'soldiers') $finalSoldiers = $newUnits;
        elseif ($u === 'guards') $finalGuards = $newUnits;
        elseif ($u === 'spies') $finalSpies = $newUnits;
        elseif ($u === 'sentries') $finalSentries = $newUnits;
