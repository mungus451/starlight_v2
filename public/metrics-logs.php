...
function parseCredentials(string $raw): array
{
    $credentials = [];
...
function ipIsAllowed(string $clientIp, string $allowlistRaw): bool
{
    if ($allowlistRaw === '') {
        return true;
    }

    $allowedIps = array_filter(array_map('trim', explode(',', $allowlistRaw)));
...
// =================== AUTH CONFIG ===================
$credentialsRaw = readEnv('METRICS_LOGS_CREDENTIALS');
$users = parseCredentials($credentialsRaw);
$allowlistRaw = readEnv('METRICS_LOGS_ALLOWLIST');
$sessionName = 'sd_admin';
// ===================================================

$clientIp = (string)($_SERVER['REMOTE_ADDR'] ?? '');
if (!ipIsAllowed($clientIp, $allowlistRaw)) {
...
if ($users === []) {
    ...
    header('Content-Type: text/plain; charset=UTF-8');
    echo 'Metrics logs auth is not configured. Set METRICS_LOGS_CREDENTIALS.';
    exit;
}
...
<div class="card">
  ...
  <form method="post" action="{$action}" autocomplete="off">
    ...
    <div class="help">Authorized Personnel Only</div>
</div>
</body>
</html>
HTML;
