...
/* @var int $allianceId */
/* @var 
App\Models\Entities\User $viewer */
...
// Helper to get alliance tag from ID
$getAllianceTag = function($id, $alliances) {
    foreach ($alliances as $a) {
        if ($a->id === $id) return htmlspecialchars($a->tag);
    }
    return '';
};

$allianceName = $getAllianceName($allianceId, $otherAlliances); // This is likely wrong, current alliance name should be from viewer->alliance_name.
// Actually, $viewer contains $viewer->alliance_name, $viewer->alliance_tag, etc.
// But $allianceId is passed, and $alliance is not. So I need to pull from $viewer or similar for current alliance info.
// The Presenter for Alliance Profile had `$formattedAlliance` but this is different endpoint.
// Let's assume $viewer->alliance_name and $viewer->alliance_tag are available.
