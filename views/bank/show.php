...
<?php if (!empty($spa_bank_enabled)): ?>
    <div
        id="bank-spa-root"
        data-credits="<?= htmlspecialchars((string)$resources->credits) ?>"
        data-banked-credits="<?= htmlspecialchars((string)$resources->banked_credits) ?>"
        data-deposit-charges="<?= htmlspecialchars((string)$stats->deposit_charges) ?>"
        data-last-deposit-at="<?= htmlspecialchars((string)($stats->last_deposit_at ?? '')) ?>"
        data-max-charges="<?= htmlspecialchars((string)$maxCharges) ?>"
        data-regen-hours="<?= htmlspecialchars((string)($bankConfig['deposit_charge_regen_hours'] ?? 6)) ?>"
        data-deposit-percent-limit="<?= htmlspecialchars((string)($bankConfig['deposit_percent_limit'] ?? 0.8)) ?>"></div>
    <script>
        window.__bankSpaMounted = false;
        window.setTimeout(() => {
            if (!window.__bankSpaMounted) {
    ...
    <script type="module" src="/spa/bank.js"></script>
<?php endif; ?>

<div id="bank-legacy-root" <?= !empty($spa_bank_enabled) ? ' style="display:none" data-spa-hidden="1"' : '' ?>>
