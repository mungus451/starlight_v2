...
<div class="mobile-content">
    <div class="player-hub" style="margin-bottom: 1rem; padding: 1rem; background: transparent; border: none; box-shadow: none;">
        <h1 style="font-family: 'Orbitron', sans-serif; font-size: 2rem; color: var(--mobile-text-primary); text-shadow: 0 0 10px var(--mobile-text-primary);">Archives</h1>
        <p style="color: var(--mobile-text-secondary); font-size: 0.9rem; margin: 0.5rem 1rem 0 1rem; text-align: center;">Recent intelligence reports.</p>
    </div>
    ...
    <div class="mobile-tabs" style="justify-content: center; margin-bottom: 1rem; gap: 0.5rem;">
        ...
        <?php foreach ([5, 10, 25, 100] as $opt): ?>
            <a href="/spy/reports?page=1&limit=<?= $opt ?>" 
    ...
    <?php if (empty($reports)): ?>
        <p class="text-center text-muted" style="padding: 2rem;">No reports on file.</p>
    <?php else: ?>
        <?php foreach ($reports as $report): 
            $isWin = ($report['result_class'] === 'res-win');
            $icon = $isWin ? 'fa-check-circle' : 'fa-times-circle';
            $color = $isWin ? 'var(--mobile-accent-green)' : 'var(--mobile-accent-red)';
        ?>
        <div class="mobile-card" style="border-left: 4px solid <?= $color ?>;">
            <div class="mobile-card-content" style="display: block; padding: 1rem;">
                ...
                <a href="/spy/report/<?= $report['id'] ?>" class="btn btn-outline" style="width: 100%; text-align: center; margin-top: 0;">
                    View Intel
                </a>
            </div>
        </div>
    ...
    <div class="mobile-tabs" style="justify-content: space-between; margin-top: 1rem;">
        <?php if ($prevPage): ?>
            <a href="/spy/reports?page=<?= $prevPage . $limitParam ?>" class="btn">
        ...
        <?php if ($nextPage): ?>
            <a href="/spy/reports?page=<?= $nextPage . $limitParam ?>" class="btn">
    ...
    <div style="margin-top: 2rem; text-align: center;">
        <a href="/spy" class="btn btn-outline" style="width: 100%;">
